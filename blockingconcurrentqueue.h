...
namespace details
{
  ...
  std::int64_t getutime() {
    ...
    double mepoch = system_clock::now().time_since_epoch() / microseconds(1);
...
public:
  ...
  // Thread-safe.
	template<typename It>
  inline void wait_enqueue_bulk(It itemFirst, size_t max)
  {
    size_t count = 0;
    while (count < max) {
		  size_t gained = (size_t)sema_enqueue->waitMany((LightweightSemaphore::ssize_t)(ssize_t)(max - count));
      while (!inner.try_enqueue_bulk(std::forward<It&>(itemFirst), gained)) {
        continue;
      }
      count += gained;
    }
  ...
  inline void wait_enqueue_bulk(producer_token_t const& token, It itemFirst, size_t max)
  {
    ...
    while (count < max) {
		  size_t gained = (size_t)sema_enqueue->waitMany((LightweightSemaphore::ssize_t)(ssize_t)(max - count));
      while (!inner.try_enqueue_bulk(token, std::forward<It&>(itemFirst), gained)) {
        continue;
      }
      count += gained;
    }
  ...
  // Thread-safe.
	template<typename It>
  inline void wait_enqueue_bulk_timed(It itemFirst, size_t max, std::int64_t timeout_usecs)
  {
    std::int64_t t0 = details::getutime(), tnow = t0;
    size_t count = 0;
    while (count < max && (tnow - t0) < timeout_usecs) {
		  size_t gained = (size_t)sema_enqueue->waitMany((LightweightSemaphore::ssize_t)(ssize_t)(max - count), tnow - t0);
      while (gained > 0 && !inner.try_enqueue_bulk(std::forward<It&>(itemFirst), gained)) {
        continue;
      }
      count += gained;
      tnow = details::getutime();
    }
  }
  ...
  inline void wait_enqueue_bulk_timed(producer_token_t const& token, It itemFirst, size_t max, std::int64_t timeout_usecs)
  {
    ...
    while (count < max && (tnow - t0) < timeout_usecs) {
		  size_t gained = (size_t)sema_enqueue->waitMany((LightweightSemaphore::ssize_t)(ssize_t)(max - count), tnow - t0);
      while (gained > 0 && !inner.try_enqueue_bulk(token, std::forward<It&>(itemFirst), gained)) {
        continue;
      }
      count += gained;
      tnow = details::getutime();
